...
model %>% 
<<<<<<< Updated upstream
  layer_simple_rnn(units = 23, input_shape = c(ncol(X_train)-1,1), activation='tanh', 
=======
  layer_simple_rnn(units = 21, input_shape = c(ncol(X_train)-1,1), activation='tanh', 
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
Eval.pd = matrix(ncol = 2, nrow = 4)
Eval.tot = matrix(ncol = 2, nrow = 4)
=======
...
df3 = list()
<<<<<<< Updated upstream
Predicciones = matrix(NA, ncol = 2, nrow = 4*3)
Index = seq(1,3*4, 3)


for (i in 1:4) {
=======
Predicciones = matrix(NA, ncol = 2, nrow = length(Cantones)*3)
...
>>>>>>> Stashed changes
  ...
  for (k in 1:(nrow(X_testc)-1)) {
    pred[k] = predice(X_testc)[k]
    X_testc[k+1,21] = pred[k]
  } 
  pred[nrow(X_testc)] = predice(X_testc)[nrow(X_testc)]
