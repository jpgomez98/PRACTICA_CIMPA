...
for (i in 1:12) {
  ...
  for (j in 1:length(Grupos[[i]])) {
    ...
    df3[[count]] = rbind(df2[[count]], df1[[count]])
   
    
    everyother1 <- function(x) x[(seq_along(Fecha) + 5)%%48 == 6]
    ...
    for (w in 1:12) {
      ...
      indx = which(df3[[count]]$Mes== w)
      x <- df3[[count]][Mes==w,1] # tomo el mes w con las predicciones para cada mes (es decir que tomo el mes w para los años que hayan reportados)
    ...
    df2[[count]] = cbind(df2[[count]], LI[1:232], LS[1:232])
    ...
    p1[[count]] = ggplot(df1[[count]], aes(x = Fecha, y = RR, group = 1)) + geom_line(colour = "blue") + 
      ...
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
            ...
            plot.title = element_text(hjust = 0.5), legend.position = "none" ) + labs (x = "", y = "") + 
      geom_ribbon(aes(ymin= LI,ymax= LS), alpha=0.2, fill = "red") +
      ggtitle(unlist(Grupos)[count])
    
    for (g in 1:32) {
    
    p2[[g]] = ggplot(df2[[g]], aes(x = Fecha, y = RR, group = 1)) + geom_line(colour = "blue") + 
      geom_line(aes(x = Fecha, y = y_pred, colour = "red"))+   
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5),
            panel.background = element_blank(), panel.border = element_rect(fill = NA, colour = 'black', size = 0.5),
            legend.position = "none" )+
      scale_x_discrete(breaks = Fecha[c(1,46,106,166,226)]) + labs (x = "", y = "") +
      geom_ribbon(data = df1[[g]], aes(ymin = LI,ymax= LS), alpha=0.5, fill = "red") +
      ggtitle(unlist(Grupos)[g])
...
```{r}
Metricas = cbind (unlist(Grupos),round(Eval.pd,3), round(Eval.tot,3))
colnames(Metricas) = c("Cantón","NMRSE 2021", "NIS 2021", "NMRSE Total", "NIS total")
Metricas = as.data.frame(Metricas)
Metricas

save(Metricas, file = "Metricas_grupal.Rdata")
write_xlsx(Metricas,"C:/Users/usuario1/Desktop/CIMPA/Github_CIMPA/PRACTICA_CIMPA/Trabajo Individual/Metricas_grupal.xlsx")
```


## Acomodo de los gráficos según calidad de predicción
...
NIS.pred = grid.arrange(p1[[16]], p1[[6]], p1[[26]], p1[[5]], p1[[7]], p1[[18]], p1[[14]], p1[[31]], p1[[32]], ncol = 3, left = "Relative Risk", bottom = "Date")

NIS.todo = grid.arrange(p2[[16]], p2[[6]], p2[[26]], p2[[5]], p2[[7]], p2[[18]], p2[[14]], p2[[31]], p2[[32]], ncol = 3, left = "Relative Risk", bottom = "Date")

ggsave(NIS.pred, filename = "C:/Users/usuario1/Desktop/CIMPA/Github_CIMPA/PRACTICA_CIMPA/Trabajo Individual/resultados_pred.pdf", width = 15, height = 10)
ggsave(NIS.todo, filename = "C:/Users/usuario1/Desktop/CIMPA/Github_CIMPA/PRACTICA_CIMPA/Trabajo Individual/resultados_todo.pdf", width = 15, height = 10)
```
